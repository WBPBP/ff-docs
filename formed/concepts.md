...
|2019.12.02|서버 아키텍쳐 내용 추가.|허설|
|2019.12.02|센서 모듈 아키텍쳐 내용 추가.|강은선|
|2019.12.02|데이터 스펙, 애플리케이션 기능 설명 추가.|정재희|
...
>### 수집한 데이터 전송
>센서로부터 수집한 데이터를 가장 효율적인 형태로 가공하여 사용자의 휴대전화로 전송한다.
...
센서는 최소 12개에서 최대 16개까지 배치되는데, 다수의 아날로그 입력을 처리하기 위해 4 by 4 input matrix를 구성하여 8개의 배선을 사용한다.
...
>### ESP-32S
>블루투스 내장 소형 MCU로, 센서로부터 값을 읽은 뒤 이를 처리하여 외부로 전송하는 역할을 수행한다.
