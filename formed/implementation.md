...
서버에서 수집하는 사용자 데이터는 이메일(ID), 비밀번호, 나이, 성별, 신장, 몸무게,걸음걸이 패턴이다.
회원가입 시 사용자는 걸음걸이패턴을 제외한 이메일(ID), 비밀번호, 나이, 성별, 신장, 몸무게를 입력한다. 해당 데이터들은 POST를 사용하여 넘겨준다.
나이, 신장, 몸무게는 integer 형식으로, 성별은 Boolean이나 integer(0 과 1 사용) 형식으로 저장한다. 이때, 나이는 10대, 20대, 30대, 40대, 50대, 60대 이상으로 분류하여 그룹화 한다. 이메일을 아이디로 사용하여 사용자에게 간편함을 제공한다. 이때, 코드에는 user_name으로 저장되며, 데이터베이스가 삽입됨과 동시에 사용자 고유의 user_id가 부여된다.
압력 데이터 분석에는 나이, 성별, 신장이 사용된다.
로그인 데이터 역시 POST로 넘겨준다. 사용자 데이터베이스에 저장되어 있는 정보와 일치하면 1을 불일치하면 0을 반환한다.
기타 정보는 GET을 사용하여 전달하고 회원 탈퇴나 정보 변경 등 사용자의 데이터를 삭제할 때는 DELETE를 사용한다. 이때, 삭제하기 전 사용자의 아이디와 비밀번호를 한 번 더 확인 후 삭제한다.
들어오는 입력데이터는 json 형식으로 받고 가공된 걸음걸이 데이터를 케이스 별로 나누어 나온 결과를 안드로이드에게 넘겨준다. 후에, 보고서 생성을 위한 결과 값이다.
데이터를 시각화하여 보여주는 사진들은 실시간으로 보이는 영상을 일정시간에 나누어 캡쳐하여 서버가 가지고 있다가 보고서 생성할 때 안드로이드에게 결과 값과 같이 넘겨준다.
